...
double pLength(point src)
{
    return sqrt(src.x * src.x + src.y * src.y + src.z * src.z);
}

point pMultiply(point src, double scalar)
{
    point dest;
    dest.x = src.x * scalar;
    dest.y = src.y * scalar;
...
double pDot(point src, point dest)
{
    return (src.x * dest.x + src.y * dest.y + src.z * dest.z);
}

point pSum(point src, point dest)
{
    point result;
    result.x = src.x + dest.x;
    result.y = src.y + dest.y;
    result.z = src.z + dest.z;
    return result;
}

point pNegate(point src)
{
    point result;
    result.x = -src.x;
    result.y = -src.y;
...
point pNormalize(point src)
{
    double pLen = pLength(src);
    point result;
    result.x = src.x / pLen;
    result.y = src.y / pLen;
    result.z = src.z / pLen;
...
point pDifference(point src1, point src2)
{
    point result;
...
void myinit()
{
  ...
  //glClearColor(253.0 / 256.0, 246.0 / 256.0, 227.0 / 256.0, 0.0);
  glClearColor(0.0 / 256.0, 43.0 / 256.0, 54.0 / 256.0, 0.0);
