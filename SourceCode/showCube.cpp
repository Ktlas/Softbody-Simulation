      ...
      for (j=0; j<=7; j++)
        for (k=0; k<=7; k++)
        {
          ...
          glBegin(GL_POINTS); // draw point
            //glColor4f(0,0,0,0); 
          glColor4f(88.0 / 256.0, 110.0 / 256.0, 117.0 / 256.0, 0.0);
            glVertex3f(jello->p[i][j][k].x,jello->p[i][j][k].y,jello->p[i][j][k].z);        
          glEnd();
          ...
          {
            //glColor4f(0,0,1,1);
            glColor4f(38.0 / 256.0, 139.0 / 256.0, 210.0 / 256.0, 1.0);
          ...
          {
            //glColor4f(0,1,0,1);
            glColor4f(133.0 / 256.0, 153.0 / 256.0, 0.0, 1.0);
          ...
          {
            //glColor4f(1,0,0,1);
            glColor4f(220.0 / 256.0, 50.0 / 256.0, 47.0 / 256.0, 1.0);
...
point getIntersection(Plane& plane, Segment& segment, bool flag) {
    point direction = pDifference(segment.end, segment.start);
    float denominator = pDot(plane.normal, direction);
    if (denominator == 0.0f) {
        // The segment is parallel to the plane, so there is no intersection
        flag = false;
        return direction;
    }
    point temp = pDifference(plane.point, segment.start);
    float t = pDot(plane.normal, temp) / denominator;
    if (t < 0.0f || t > 1.0f) {
        // The intersection point is outside the segment, so there is no intersection
        flag = false;
        return direction;
    }
    flag = true;
    return pSum(segment.start, pMultiply(direction, t));
}

std::vector<point> getIntersectionPolygon(Plane& plane, AABB& aabb) {
    std::vector<point> points;
    // front face
    Segment tempSegment = { aabb.min, {aabb.max.x, aabb.min.y, aabb.min.z} };
    bool flag = true;
    getIntersection(plane, tempSegment, flag);
    if (flag) {
        points.push_back(getIntersection(plane, tempSegment, flag));
    }
    tempSegment = { {aabb.max.x, aabb.min.y, aabb.min.z}, {aabb.max.x, aabb.max.y, aabb.min.z} };
    getIntersection(plane, tempSegment, flag);
    if (flag) {
        points.push_back(getIntersection(plane, tempSegment, flag));
    }
    tempSegment = { {aabb.max.x, aabb.max.y, aabb.min.z}, {aabb.min.x, aabb.max.y, aabb.min.z} };
    getIntersection(plane, tempSegment, flag);
    if (flag) {
        points.push_back(getIntersection(plane, tempSegment, flag));
    }
    tempSegment = { {aabb.min.x, aabb.max.y, aabb.min.z}, {aabb.min.x, aabb.min.y, aabb.min.z} };
    getIntersection(plane, tempSegment, flag);
    if (flag) {
        points.push_back(getIntersection(plane, tempSegment, flag));
    }
    // back face
    tempSegment = { aabb.min, {aabb.max.x, aabb.min.y, aabb.max.z} };
    getIntersection(plane, tempSegment, flag);
    if (flag) {
        points.push_back(getIntersection(plane, tempSegment, flag));
    }
    tempSegment = { {aabb.max.x, aabb.min.y, aabb.max.z}, {aabb.max.x, aabb.max.y, aabb.max.z} };
    getIntersection(plane, tempSegment, flag);
    if (flag) {
        points.push_back(getIntersection(plane, tempSegment, flag));
    }
    tempSegment = { {aabb.max.x, aabb.max.y, aabb.max.z}, {aabb.min.x, aabb.max.y, aabb.max.z} };
    getIntersection(plane, tempSegment, flag);
    if (flag) {
        points.push_back(getIntersection(plane, tempSegment, flag));
    }
    tempSegment = { {aabb.min.x, aabb.max.y, aabb.max.z}, {aabb.min.x, aabb.min.y, aabb.max.z} };
    getIntersection(plane, tempSegment, flag);
    if (flag) {
        points.push_back(getIntersection(plane, tempSegment, flag));
    }
...
{
    //double length = sqrt(jello->a * jello->a + jello->b * jello->b + jello->c * jello->c);
    //Plane plane { {jello->a / length, jello->b / length, jello->c / length}, {-jello->d * jello->a / (length * length), -jello->d * jello->b / (length * length), -jello->d * jello->c / (length * length)} };
    //AABB aabb{ {-2, -2, -2}, {2, 2, 2} };
    //std::vector<point> points = getIntersectionPolygon(plane, aabb);

    //glColor4f(211.0 / 256.0, 54.0 / 256.0, 130.0 / 256.0, 0.5);
    //glBegin(GL_LINES);
...
void showBoundingBox()
{
  ...
  glColor4f(238.0 / 256.0, 232.0 / 256.0, 213.0 / 256.0, 0.0);
